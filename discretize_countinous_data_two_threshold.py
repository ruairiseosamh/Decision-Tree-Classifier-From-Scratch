...
def calculate_gain(my_dataset, my_first_threshold, my_second_threshold, global_entropy, target):
    ...
    for _, row in my_dataset.iterrows():
        if(row['colour'] <= my_first_threshold):
            if(row['style']==target):
        ...
        elif(row['colour'] >= my_first_threshold and row['colour'] <= my_second_threshold):
            if(row['style']==target):
...
pairs =  pd.DataFrame(df,columns=["colour", "style"])
# print(X.head())
# print(y.head())
sorted = pairs.sort_values('colour', ascending=True)
#print(sorted.head())
#print(sorted.tail())
highest_gain = 0
...
for target in targets:
    ...
    for x in np.arange(math.ceil(sorted.head(1)['colour'].values), math.floor(sorted.tail(1)['colour'].values), 0.1):
        for z in np.arange(math.ceil(sorted.head(1)['colour'].values), math.floor(sorted.tail(1)['colour'].values), 0.1):
            #if(x<y):
            resulting_gain = calculate_gain(sorted, x, z, global_entropy, target)
            if resulting_gain > highest_gain:
                highest_gain = resulting_gain
                best_threshold_x = x
                best_threshold_z = z

    print("highest information gain for " + target  + " achieved is " + str(highest_gain))
